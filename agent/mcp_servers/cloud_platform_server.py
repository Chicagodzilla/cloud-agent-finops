...
try:
    import pymysql
except ImportError:  # pragma: no cover - exercised by mock-mode imports
    pymysql = None
...
def get_db_connection():
    ...
    if pymysql is None:
        raise RuntimeError("pymysql 未安装，请启用 MOCK_DATA_MODE=true 或安装 MySQL 依赖。")
...
def use_mock_data() -> bool:
    """Default to local mock data so the open-source demo works out of the box."""
    return os.getenv("MOCK_DATA_MODE", "true").strip().lower() not in {"0", "false", "no"}
...
def query_user_orders(user_id: str, limit: int = 5) -> str:
    ...
    """
    if use_mock_data():
        return dumps(mock_data_provider.query_user_orders(user_id=user_id, limit=limit))
...
def query_user_instances(user_id: str, limit: int = 5) -> str:
    ...
    """
    if use_mock_data():
        return dumps(mock_data_provider.query_user_instances(user_id=user_id, limit=limit))
...
def analyze_instance_usage(instance_id: str, user_id: str = "") -> str:
    ...
    if use_mock_data():
        return dumps(mock_data_provider.analyze_instance_usage(instance_id=instance_id, user_id=user_id))
...
@mcp.tool()
def query_monthly_bill_summary(user_id: str, billing_month: str = "2026-05") -> str:
    """
    查询用户指定月份的账单汇总，返回总金额和按产品类型拆分的费用。
    ...
    if use_mock_data():
        return dumps(
            mock_data_provider.query_monthly_bill_summary(
                user_id=user_id,
    ...
    try:
        ...
        with connection.cursor() as cursor:
            ...
            return dumps({
                ...
                "source": "mysql.cloud_orders",
            })
    except Exception as e:
        return dumps({"status": "error", "message": f"查询账单汇总失败: {str(e)}"})
    finally:
        if 'connection' in locals() and connection.open:
            connection.close()
...
def query_resource_cost_breakdown(
    ...
    Args:
        resource_id: 资源 ID，如 ECS instance_id。
        user_id: [系统注入] 当前用户 ID。
        billing_month: 账单月份，格式如 "2026-05"。
    """
    if use_mock_data():
        return dumps(
            mock_data_provider.query_resource_cost_breakdown(
    ...
    return dumps({
        ...
        "message": "当前 MySQL 表结构暂未提供资源级费用字段，请启用 Mock 模式或扩展账单表。",
        "data": None,
    })


@mcp.tool()
def query_instance_metrics(instance_id: str, user_id: str = "", days: int = 7) -> str:
    """
    查询实例每日监控明细，用于生成资源诊断和成本优化依据。

    Args:
        instance_id: 实例 ID，必须先通过 query_user_instances 查询获得。
        user_id: [系统注入] 当前用户 ID。
        days: 查询最近多少天，默认 7 天。
    """
    if use_mock_data():
        return dumps(
            mock_data_provider.query_instance_metrics(
                instance_id=instance_id,
                user_id=user_id,
                days=days,
            )
    ...
    return dumps({
        "status": "not_supported",
        "message": "当前 MySQL 工具仅支持聚合监控分析，请使用 analyze_instance_usage。",
        "data": [],
    })
...
def estimate_savings(
    ...
    """
    估算将实例降配到目标规格后的月度节省金额。任何实际变更都需要人工确认。

    Args:
        instance_id: 实例 ID，必须属于当前用户。
        target_instance_type: 目标实例规格，如 "ecs.g8a.xlarge"。
        user_id: [系统注入] 当前用户 ID。
    """
    if use_mock_data():
        return dumps(
            mock_data_provider.estimate_savings(
                instance_id=instance_id,
    ...
    return dumps({
        "status": "not_supported",
        "message": "当前 MySQL 模式暂未配置价格目录，无法估算节省金额。",
        "data": None,
    })
